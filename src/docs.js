...
<div class="wrap">
  ...
  <p>Space is an 8 point grid, but templates never name a number, they name a relationship: <code>stack.related</code> 24, <code>stack.group</code> 32, <code>stack.section</code> 48, with a separate inline scale for items on one line. Two rules govern the values. Related things sit closer than unrelated things, and a gap between two elements must exceed the leading inside them, since a headline eight pixels above its deck reads as one block when the deck's own lines are fourteen apart.</p>
  <p>The deck clamps to one line and the headline takes the rest, because truncating a qualifier costs less than truncating the point. How many lines the headline gets is a property of each frame and is measured against the card rather than chosen.</p>
  <p>Every value a template uses is recorded and checked: the build fails if a size is off the scale, if a space is not a named token, if a gap does not clear its leading, if a declared token goes unused, if anything reads below 11 point at preview scale, or if any template overflows its own card.</p>
